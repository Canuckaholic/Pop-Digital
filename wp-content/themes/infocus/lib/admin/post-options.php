 ...
 */
function webtreats_meta_boxes($meta_name = false) {
	global $themename;

	$meta_boxes = array(
'generalpost' => array(
	'id' => 'webtreats_generalpost_meta',
	'title' => $themename . ' General Post Options',
	'function' => 'webtreats_generalpost_meta_box',
	'noncename' => 'webtreats_generalpost',
...
'generalpage' => array(
	'id' => 'webtreats_generalpage_meta',
	'title' => $themename . ' General Page Options',
	'function' => 'webtreats_generalpage_meta_box',
	'noncename' => 'webtreats_generalpage',
...
'portfolio' => array(
	'id' => 'webtreats_portfolio_page_meta',
	'title' => $themename . ' Portfolio Page Options',
	'function' => 'webtreats_portfolio_page_meta_box',
	'noncename' => 'webtreats_portfolio',
...
function webtreats_add_meta_box($box_name) {
global $post;

$meta_box = webtreats_meta_boxes($box_name);
...
?>
<div id="<?php echo $meta_id; ?>" class="webtreats-post-control">
...
function webtreats_save_meta($post_id) {
	$meta_boxes = webtreats_meta_boxes();
	
	$svr_uri = $_SERVER['REQUEST_URI'];
	foreach($meta_boxes as $meta_box) {
		if ( strstr($svr_uri, 'post.php') || strstr($svr_uri, 'post-new.php') ) {	
			if (!wp_verify_nonce($_POST['webtreats_generalpost_noncename'], plugin_basename(__FILE__)))
				return $post_id;
			if (!wp_verify_nonce($_POST['webtreats_portfolio_noncename'], plugin_basename(__FILE__)))
				return $post_id;
		}

		if ( strstr($svr_uri, 'page.php') || strstr($svr_uri, 'page-new.php') ) {	
			if (!wp_verify_nonce($_POST['webtreats_generalpage_noncename'], plugin_basename(__FILE__)))
				return $post_id;
		}
	}
...
function webtreats_generalpost_meta_box() {
	webtreats_add_meta_box('generalpost');
}

function webtreats_generalpage_meta_box() {
	webtreats_add_meta_box('generalpage');
}

function webtreats_portfolio_page_meta_box() {
	webtreats_add_meta_box('portfolio');
}

function webtreats_add_meta_boxes() {
	$meta_boxes = webtreats_meta_boxes();
	...
	add_action('save_post', 'webtreats_save_meta');
}
	add_action('admin_menu', 'webtreats_add_meta_boxes')
?>