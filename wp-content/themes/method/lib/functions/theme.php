		...
		if( $intro_text != 'default' ) {
			...
			if( in_array( $intro_text, array( 'title_only', 'title_teaser', 'title_tweet' ) ) ) {
				...
				elseif( is_singular( 'portfolio' ) )
					$title = __('Work', MYSITE_TEXTDOMAIN );
	...
	if ( is_archive() ) {
		...
		if( $intro_options != 'disable' ) {
			...
			if( is_category() ) {
				...
				$text = sprintf( __('All Posts Tagged with: %1$s', MYSITE_TEXTDOMAIN ), '&lsquo;' . single_tag_title('',false) . '&rsquo;');
			} elseif ( is_day() ) {
...
function mysite_page_title() {
	...
	if ( is_search() ) {
		...
		$title = sprintf( __('All Posts Tagged with: %1$s', MYSITE_TEXTDOMAIN ), '&lsquo;' . single_tag_title('',false) . '&rsquo;');
	} elseif ( is_day() ) {
